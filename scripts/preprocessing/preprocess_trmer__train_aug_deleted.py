...
def process_args():
    ...
    parser.add_argument('--img', dest='img_path',
                        default=root_path + '/data/image_processed_data/no_aug/20K/images_deleted',
                        type=str,
...
if __name__ == '__main__':
    for which_set in which_sets:
        ...
        for i in tqdm(range(len(labelEmbed_order_by_len))):
            ...
            imgd = Image.open(parameters.img_path +
                             f_list[item_f_list].strip() + 'd.png').convert('L')
            imgs = Image.open(parameters.img_path +
                             f_list[item_f_list].strip() + 's.png').convert('L')
            imgp = Image.open(parameters.img_path +
                             f_list[item_f_list].strip() + 'p.png').convert('L')
            # if i == 16000:
            #     img.show()
            for item in (img,imgd,imgs,imgp):
        ...
        all_data = (imgList, labelEmbed_teaching_ordered_array.repeat(4,axis=0), labelEmbed_predict_ordered_array.repeat(4,axis=0))
        out_file = root_path + '/data/numpy_processed_data/tri_aug/20K/' + which_set + '.npy'
        np.save(out_file, np.array(all_data))
        print('Save {} dataset to {}'.format(which_set, out_file))
