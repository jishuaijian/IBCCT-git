    ...
    for which_set in which_sets:
        ...
        for i in tqdm(range(len(labelEmbed_order_by_len))):
            ...
            for item in (img,imgd,imgs,imgp):
                item_np_array = delete_Padding(np.array(item))

                tmp_img = Image.fromarray(item_np_array)
                resize = transforms.Resize([112, 224])
                tmp_img = resize(tmp_img)
                #plt.imshow(im_pad, cmap='gray')
                #plt.show()
                imgList.append(np.array(tmp_img))
        ...
        print('Save {} {} dataset to {}'.format(which_set,len(imgList),out_file))
