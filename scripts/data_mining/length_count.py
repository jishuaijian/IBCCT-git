...
def process_args():
    ...
    parser.add_argument('--img', dest='img_path',
                        default=root_path + '/data/image_processed_data/tri_aug/20K/images_processed/',
                        type=str,
...
if __name__ == '__main__':
    for which_set in which_sets:
        ...
        sum_longger_max_length = sum(1 for item in Label_len_list if item>MAXLENGTH)
        if which_set == 'train':
            #print('the 70229 length is {}'.format(len(labelEmbed_teaching_ordered_array[70228])))
            pass
        if which_set =='val':
            #print('the 8749 length is {}'.format(len(labelEmbed_teaching_ordered_array[8749])))
            pass
        print('{} the max_length is {}'.format(which_set, len(labelEmbed_teaching_ordered_array[-1])))
        print('{} the number of lengger than max_length is {}'.format(which_set,sum_longger_max_length))